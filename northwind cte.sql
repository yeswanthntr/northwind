...
--- Q1. Total Sales by Customer
with total_sales as (
select so.custid,
	   sum(od.unitprice * od.quantity * (1-discount)) as totalsales
       from salesorder so inner join orderdetail od on so.orderid=od.orderid group by so.custid) select * from total_sales;
       
       
       
       
--- 2Find the customers whose total sales are greater than the average total sales of all customers.
select * from salesorder;
select * from orderdetail;  
with total_sales as (
 select so.custid, sum(od.unitprice * od.quantity * (1-od.discount)) as totalsales
 from salesorder so inner join orderdetail od on so.orderid=od.orderid group by so.custid),
 average_sales as ( select avg(totalsales) as avgs from total_sales)
 select ts.custid ,ts.totalsales from total_sales ts cross join average_sales av where ts.totalsales > av.avgs;
 
--- 3 Find the top 5 products by total sales.
select * from orderdetail;
select  * from product;
with pro as (select p.productname,sum(od.unitprice * od.quantity * (1-discount)) as totalsale
from product p inner join orderdetail od on p.productid=od.productid group by p.productid,p.productname)
select * from pro;

--- 4Find the total sales for each month.
select * from salesorder;
select * from orderdetail; 

with monthly_sales as ( select sum(od.unitprice * od.quantity * (1-discount)) as totalsales,
MONTH(so.orderDate) as month from salesorder so inner join orderdetail od on so.orderid=od.orderid
group by month(so.orderdate))
select * from monthly_sales group by month;

--- 5 Find the total sales made by each employee.
select * from salesorder;
select * from orderdetail; 
select * from employee;


with employee_sales as (select e.employeeid,e.firstname,e.lastname,sum(od.unitprice * od.quantity * (1-discount)) as totalsales
from employee e inner join salesorder so on e.employeeid=so.employeeid inner join orderdetail od on so.orderid=od.orderid
group by e.employeeid,e.firstname,e.lastname)
select * from employee_sales order by employeeid asc;



--- 6 Find the products whose unit price is greater than the average unit price of all products.
select * from product;



WITH avg_price AS (
    SELECT AVG(unitPrice) AS avgs
    FROM Product
),
products AS (
    SELECT
...
SELECT
    ...
    p.unitPrice
FROM products p
CROSS JOIN avg_price av
WHERE p.unitPrice > av.avgs;
...
--- 9 Find the highest-selling product in each category.
...
--- 10 Rank customers based on their total sales, from highest to lowest.
select * from salesorder;
select * from orderdetail;
